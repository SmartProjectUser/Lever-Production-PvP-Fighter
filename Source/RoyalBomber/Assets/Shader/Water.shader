Shader "Custom/Water" {
	...
	SubShader{
		...
		Pass{
			...
			uniform float _LMPower;
			uniform half4 _LMColor;

			uniform fixed _DontMix;

			uniform fixed _LM;

			static const half3 FrontDir = half3(0, 0.2, 1);
			static const half3 BackDir = half3(0, 0.5, -1);
			static const half3 LeftDir = half3(1, 0.5, 0);
			static const half3 RightDir = half3(-1, 0.5, 0);
